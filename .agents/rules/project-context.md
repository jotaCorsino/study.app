...
- PR #12 final player polish is now wired in runtime: the left `Meus Cursos` daily indicator refreshes live via `IProgressService` notifications when lesson minutes are credited, the lesson-header previous button is icon-only with accessibility labels, and lesson controls keep `Marcar como concluida` plus inline speed/intro actions on one row until a truly narrow viewport requires wrapping
