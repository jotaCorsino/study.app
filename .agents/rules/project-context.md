...
- Lesson player v1 speed-control foundation is now wired for local playback only: `LessonPlayer` exposes a session-memory selector (`0.5x`, `1x`, `1.5x`, `2x`, `2.5x`), the selected speed propagates through `LessonPlaybackHost` to `LocalVideoPlayer`, `NativeLessonPlaybackService` now tracks per-session `PlaybackSpeed`, and `MainPage` applies speed safely on the native `MediaElement` without implementing click-to-toggle or external YouTube speed yet
- Local lesson playback now supports tap-to-toggle on the native host (`MainPage`): single tap pauses when playing and resumes when paused/ready, guarded by session/lifecycle/source-transition checks and a reserved bottom interaction zone to avoid conflicts with native transport controls; external YouTube playback remains unchanged
- External YouTube playback now has playback-speed bridge support: the lesson speed selection is propagated to the external runtime, the host applies rate by best effort via YouTube IFrame API, and the app tracks both requested and effective rates per session (including fallback when `2.5x` is unavailable); external tap-to-toggle remains intentionally out of scope
- Player QA hardening pass applied: external requested/effective speed tracking now preserves real effective fallback values from YouTube (including non-menu rates), the lesson UI now shows a clear fallback hint when effective speed differs from requested on external lessons, and local tap-to-toggle now caps the native controls exclusion zone proportionally to avoid disabling toggle in very small host heights
...
- Local-course rehydration is now manifest-driven and persistence-safe: `PersistedCourseService` validates local persisted structure against `course_import_snapshots.structure_json`, rebuilds missing module/topic/lesson trees from the stored manifest when a local course returns partial, preserves lesson progress/current selection during rebuild, and creates a non-destructive legacy manifest fallback on first load when older local courses have no snapshot yet
- Local lessons now persist a portable `RelativeFilePath` alongside the existing absolute `LocalFilePath`: new folder imports and manifest rehydration preserve scanner-relative `/` paths, SQLite schema v11 backfills only safe in-root legacy paths without requiring files to be connected, and the current player behavior remains unchanged
- Local lesson playback now resolves its effective physical path through a central stateless resolver: the loaded course root plus a safe `RelativeFilePath` takes priority, `LocalFilePath`/`FilePath` remain compatibility fallbacks, and the player propagates the current root and reactivates the same lesson whenever its effective path changes, without any schema or persistence change
- Local-folder courses now have a backend source-management service that validates candidate folders by portable relative lesson paths, ignores scanner IDs that depend on the absolute root, classifies exact/new-content/partial/incompatible matches, and atomically rebases the course root, safe legacy lesson paths, and import snapshot while preserving persisted identities, progress, presentation, and metadata; newly detected content is reported but not synchronized in this phase
...
- StudyHub `v1.2.0` is published with `ApplicationDisplayVersion=1.2.0` and monotonic `ApplicationVersion=14`; the annotated tag resolves to binary commit `a7d277649da8f3401f555c1eceb4b15c237c2900`, and the public `studyhub-windows-x64.zip` was re-downloaded, hash-verified, extracted, and smoke-tested successfully.
