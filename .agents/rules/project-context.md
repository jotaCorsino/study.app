...
- Windows native transport-navigation hotfix applied in `MainPage`: lesson navigation now listens to both `MediaPlaybackCommandManager` and `SystemMediaTransportControls.ButtonPressed` for `Next`/`Previous`, suppresses duplicated commands in a short per-session window, and detaches all related handlers during page unload/detach
